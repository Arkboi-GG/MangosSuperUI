...
> ⚠️ **Process Name vs Binary Name:** The file on disk may be named `mangosd`, but once running, the OS may report the process as `mangosd-main` (or vice versa). This matters because MangosSuperUI detects running processes by name. You can discover the actual process name with `cat /proc/$(pgrep -f mangosd)/comm` while the server is running. MangosSuperUI v1.1+ auto-scans `/proc` as a fallback, and the Dashboard's **Diagnose** button will detect and report this mismatch automatically.
...
If anything is red or showing unexpected values, click the **Diagnose** button in the top-right corner of the Dashboard. This runs a comprehensive self-test that probes every subsystem and tells you specifically *why* something is broken — not just that it is. Each failed check includes a copy-pasteable fix command.

The Diagnose feature checks: configuration status, process detection (configured name vs actual `/proc/comm` name), RA connectivity (distinguishes "port closed" from "port open but auth failed"), mangosd.conf parsing (checks for `Ra.Enable = 1` and `Ra.MinLevel = 3`), all database connections (with specific error codes), server paths with file counts, and static asset directories.
...
> ⚠️ **Settings Save and Restart:** Most settings (DB connections, RA credentials) apply immediately. However, **path settings** used by services like PatchBuilderService (ClientM2Path, ClientDataPath, PatchOutputPath) may not hot-reload reliably on Linux. If you change path settings and things aren't working, restart: `sudo systemctl restart mangossuperui`

#### Spell Creator & World Viewer Paths (Optional)
...
## Part 4: Spell Creator & World Viewer Assets (Optional)

These steps are only needed if you use the Spell Creator (custom spell visual effects) or the 3D World Viewer (terrain rendering from client MPQs).

---

### Step 21: Copy WoW Client Data to the Server
...
Then set `Vmangos:ClientDataPath` in Settings to `/home/YOUR_USERNAME/wowclient/Data`.

---

### Step 22: Extract Spell M2 Files

The Spell Creator needs pre-extracted M2 particle effect files. These are extracted on the Linux server using a python script with the `mpyq` library.

```bash
pip3 install --user mpyq --break-system-packages
```
...
m2_dir = os.path.expanduser('~/wowclient/m2')
data_dir = os.path.expanduser('~/wowclient/Data')
...
Then set `Vmangos:ClientM2Path` in Settings to `~/wowclient/m2` (use the full path).

> ⚠️ **After setting paths in Settings, restart MangosSuperUI:** `sudo systemctl restart mangossuperui`. Path settings used by PatchBuilderService don't always hot-reload on Linux.

---

### Step 23: Extract Vanilla BLPs

The Spell Creator's texture pipeline uses vanilla BLP headers to match output dimensions and compression format. Without these, custom textures default to 512×512 DXT3 which can cause visual artifacts.

```bash
python3 << 'PYEOF'
import mpyq, os

blp_dir = os.path.expanduser('~/wowclient/rawblps/m2_blps')
data_dir = os.path.expanduser('~/wowclient/Data')
...
Expected: ~2,900 BLP files. Then set `SpellCreator:RawBlpPath` in Settings and restart.

---

## Part 5: Validation Script (Optional)

A comprehensive validation script is included in the repository at `Scripts/validate-mangossuperui.sh`. It audits your entire installation — checks all paths and connections, validates file counts, detects missing assets, and reports exactly what needs attention.

```bash
sudo bash Scripts/validate-mangossuperui.sh
```

Requires `jq` (`sudo apt install jq`). If Client Data is available and M2/BLP files are missing, the script can auto-extract them using the same python `mpyq` scripts from Part 4.
